...
class Network:
    def __init__(self, networkGraph=None, positionSettings={}, mobilitySettings={}, appSettings={}, initEnergyJ=100,
        ...
        self.mobilityHelper, self.predictor = self.buildNetworkFromGraph(networkGraph, mobilitySettings, verbose)
    ...
    def buildNetworkFromGraph(self, networkGraph, mobilitySettings, verbose):
        ...
        #mobilityModel = "ns3::ManhattanMobilityTargetPointModel"
        #mobilityModel = "ns3::ManhattanMobilityHighwayModel"

        mobilityHelper.SetMobilityModel(mobilityModel)
        mobilityHelper.Install(self.nodeContainer)
        mobilityHelper.LinkMobilityModels()
        #predictor = ns.mobility.IntersectionProbabilityPredictor()
        predictor = ns.mobility.SimpleNodeProbabilityPredictor()

        return mobilityHelper, predictor
    ...
    def initIpv6(self, prefix=2001, verbose=False, **unused_settings):
        if verbose:
            print(
                f"Initiating ipv6 stack with prefix {prefix} on network with {self.nodeContainer.GetN()} devices, assignin to {self.sixLowPanContainer.GetN()} sixlowpan devices")
    ...
    def InstallTaskApps(self, verbose=False, enable_errors=True, error_scale=1.0, error_shape=1.0, network_status=[],
                        **unused_settings):
        if verbose:
        ...
        # taskHelper.EnablePcapAll("taskapps", True)
    ...
    def getNodePositions(self, node_list):
        for i in range(self.mobilityHelper.NodesLength()):
...
def createTasksFromGraph(network, taskGraph, allocation, verbose=False, **unused_settings):
    ...
    assert ((len(taskGraph.nodes())) == len(
        allocation)), f"Task and allocation list length mismatch: {len(taskGraph.nodes())} vs {len(allocation)}"
    try:
...
def evaluate(allocation=[], repeat=False, **kwargs):
    ...
    if len(kwargs['next_alloc']) > 0 and repeat:
        ns.core.Simulator.Stop(ns.core.Time(ns.core.Seconds(3000)))
    ns.core.Simulator.ScheduleDestroy(getTime, time)
    ns.core.Simulator.Run()
    ns.core.Simulator.Destroy()
    # print(latency_list)
    ...
    if repeat:
        if np.mean(time) >= 20 and len(kwargs['next_alloc']) > 0:
            ...
            if not (checkIfAlive(**kwargs)):
                raise exceptions.NetworkDeadException
            try:
                time1, latency1, received1, energy_list1, node_status1, missed1 = evaluate(kwargs['next_alloc'],
                                                                                           repeat=False, **kwargs)
            except Exception as e:
